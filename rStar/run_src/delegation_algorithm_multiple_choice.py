...
class DelegationAlgorithm:
    def __init__(self, learning_rate: float = 0.3, max_iterations: int = 5):
        """
        ...
        if DISCRIMINATOR_AVAILABLE:
            try:
                ...
                model_name = "Qwen/Qwen2-7B-Instruct"  # ~345M parameters, fast and efficient
                ...
                self.discriminator_tokenizer, self.discriminator_model = load_HF_model(
                    model_name, 
                    YOUR_HUGGINGFACE_TOKEN_HERE= "token" # "YOUR_HUGGINGFACE_TOKEN_HERE"  # Public model
                )
    ...
    def load_gsm8k_questions(self, questions_file: str = "./data/GSM8K/test_all.json") -> Dict[int, str]:
        ...
        try:
            ...
            # Verify we have the questions we need (0-99)
            missing_questions = []
            for i in range(100):
            ...
            if missing_questions:
                ...
                print(f"✅ All questions 0-99 are available")
                ...
                if question_result['final_results']:
                    ...
                    for gen_name, result in question_result['final_results'].items():
                        ...
                        if golden_answer != 'Unknown':
                            # Check if answer was correct before delegation
                            if self._check_answers_equiv(result['answer'], golden_answer):
                                gen_metrics['correct_answers_after'] += 1
                                overall_metrics['total_correct_answers'] += 1
                            
                            # Check if answer was correct before delegation (using original confidence)
                            # We need to check the original best answer from the generator
                            try:
                                original_gen_data = generators_data[gen_name]
                                if str(question_idx) in original_gen_data:
                                    original_answer, _, _ = self.get_highest_confidence_answer(original_gen_data, question_idx)
                                    if self._check_answers_equiv(original_answer, golden_answer):
                                        gen_metrics['correct_answers_before'] += 1
                            except Exception:
                                pass  # Skip if we can't determine original answer
    ...
    def generate_summary_report(self, results: Dict) -> str:
        ...
        report.append("=" * 60)
        report.append(f"{'Generator':<25} {'Before':<15} {'After':<15} {'Change':<15}")
        report.append(f"{'':<25} {'Accuracy':<15} {'Accuracy':<15} {'(+/-)':<15}")
        report.append("-" * 60)
        ...
        for gen_name, gen_metrics in metrics['generator_performance'].items():
            ...
            report.append(f"{gen_name:<25} {accuracy_before:>6.1f}%      {accuracy_after:>6.1f}%      {change_symbol} {accuracy_change:+5.1f}%")
    ...
    def discriminator_score_answer_path(self, answer: str, solution_path: str, 
        ...
        """
        # For multiple choice approach, we need all candidates to compare
        # This method will be called from the main delegation loop with all candidates
...
def main():
    ...
    # Show question context status
    if delegation.question_contexts:
        print(f"📚 Question context loaded: {len(delegation.question_contexts)} questions available")
    else:
        print("⚠️  No question context available - will use empty context")
    
    # Load generator data
    try:
        data = delegation.load_generator_data('./llm_answers_discriminator_0_99_qwen.json')
    ...
    # Run delegation for questions 100-300
    test_questions = (100, 300)  # Process questions 100-299
    ...
    try:
        ...
        for question_idx in sorted([k for k in results.keys() if not str(k).startswith('_')]):
            ...
            for gen_name, gen_result in question_result['final_results'].items():
                ...
                print(f"  {gen_name}: {change_symbol} {gen_result['final_confidence']:.3f} "
        ...
        print(f"\n📊 DISCRIMINATOR CACHE STATISTICS:")
        print(f"  Questions cached: {cache_stats['total_questions_cached']}")
        print(f"  Total entries: {cache_stats['total_entries']}")
        print(f"  Estimated cache size: {cache_stats['cache_size_mb']:.3f} MB")
