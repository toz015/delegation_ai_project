...
try:
    ...
    from eval_src.Evaluator import MATHEvaluator  # Import the specific evaluator
...
class DelegationAlgorithm:
    def __init__(self, learning_rate: float = 0.1, max_iterations: int = 5):
        ...
        if DISCRIMINATOR_AVAILABLE:
            try:
                # Create MATH evaluator for math problems
                self.evaluator = MATHEvaluator()
                print("✅ MATH Evaluator loaded successfully")
            except ImportError:
                print("⚠️  MATH Evaluator not available, using fallback")
            ...
            if not hasattr(self, '_current_discriminator'):
                ...
                class SimpleArgs:
                    def __init__(self):
                        ...
                        self.device = "cuda"  # Specify CUDA device
                        self.tensor_parallel_size = 1  # Single GPU
...
def main():
    ...
    try:
      ...
      #  print(f"Generators: {', '.join(data['results'].keys())}")
        ...
        print("❌ Error: ./llm_answers_discriminator_MATH_0_298_Falcon_7B_Zephyr_7B_Beta_Mistral_7B_Instruct_v0.2.json not found!")
